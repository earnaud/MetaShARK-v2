...
customUnits <- function(input, output, session,
    ...
    if(any(dim(rv$CU_Table) == 0)){
      sapply(names(savevar$emlal$templateDP), function(file_name){
        ...
        if(any(tmp$unit == "custom"))
          rv$attributesNames <- c(rv$attributesNames,
  ...
  observeEvent(rv$attributesNames,{
    isolate({
      sapply(1:length(rv$attributesNames),
             function(i){
  ...
  observeEvent(rv$current_attribute,{
    rv$current_index <- match(rv$current_attribute,
                              rv$attributesNames)
  })
  ...
  output$edit_CU <- renderUI({
    ...
    tagList(
      ...
      lapply(rv$ui, function(colname) {
        ...
        # UI 
        switch(colname,
               id = textInput(ns(colname), label = colname,
                              placeholder = "e.g. milligramsPerGram",
                              value = saved_value),
               unitType = textInput(ns(colname), label = colname,
                                    placeholder = "e.g. mass",
                                    value = saved_value),
               parentSI = selectInput(ns(colname), label = colname,
                                      choices = globals$FORMAT$UNIT[-1],
                                      selected = saved_value),
               multiplierToSI = numericInput(ns(colname), label = colname,
                                             value = 1),
               description = textAreaInput(ns(colname), label = colname,
  ...
  observeEvent({
    ...
    req(unlist(reactiveValuesToList(rv$attributes)),
        rv$CU_Table)
    # save metadata
