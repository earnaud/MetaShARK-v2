#' @title templateDPUI
#'
#' @description UI part of the EML AL module (step 3: fill in
#' the data attributes)
#'
#' @param title page title
#' @param dev logical. Shall the dev items appear?
#'
#' @importFrom shiny fluidPage HTML tagList uiOutput NS icon
...
#' @title templateDP
#'
#' @description server part of the EML AL module (step 3: fill in
#' the data attributes)
#'
...
#' renderText validate need removeModal observe textAreaInput span
#'  eventReactive isTruthy
...
templateDP <- function(input, output, session, savevar, globals) {
  ...
  callModule(
    nextTab, "nav",
    globals, globals$EMLAL$PREVIOUS
  )
    ...
    tagList(
      ...
      lapply(rv$ui, function(colname) {
        ...
        if (grepl("date", colname)
            && !grepl("!Add datetime specifier here!", saved_value))
          saved_value <- gsub("^(.*) .*)$", "\\1", saved_value)
  ...
  observe({
    ...
    # Save: table to save and savevar's table must be the same
    if(identical(
      savevar$emlal$templateDP[[rv$current_file]]$attributeName,
      rv$attributesTable$attributeName
    ))
      savevar$emlal$templateDP[[rv$current_file]] <- rv$attributesTable
    # cpltTrigger$trigger()
  })
  ...
  # Process data ----
  # Idea: the nextStep variable is a collection of X numbers (X being
  # the number of facultative modules after this one). If nextStep[i]
  # equals:
  # - 1: nextStep[i] is added to globals$EMLAL$NAVIGATE to skip the 
  # module
  # - 0: nextStep[i] is added to globals$EMLAL$NAVIGATE to read the 
  # module
  # In any case, just adding nextStep[i] is enough to program a way
  # through modules
  ...
  observeEvent(input[["nav-nextTab"]],
               {
                 ...
                 nextStep <- c(1,1) # 2 modules thereafter, default: skip
                 nextStep <- sapply(rv$files_names, function(fn, nextStep) {
                   # write filled tables
                   ...
                   if ("custom" %in% savevar$emlal$templateDP[[fn]][, "unit"]) {
                     nextStep[1] <- 0
                   } # custom units
                   if ("categorical" %in% savevar$emlal$templateDP[[fn]][, "class"]) {
                     nextStep[2] <- 0
                 ...
                 globals$EMLAL$NAVIGATE <- globals$EMLAL$NAVIGATE + 
                   as.numeric(globals$EMLAL$PREVIOUS[2])
               },
               priority = 1
  )
  
  observeEvent(input[["nav-prevTab"]], {
    globals$EMLAL$PREVIOUS <- "template"
  })
