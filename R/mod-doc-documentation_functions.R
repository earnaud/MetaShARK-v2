#' @title documentation_functions.R
#'
#' @description  extracts the targetted EML content from a node. This node is
#' selected in the documentation module
#'
#' @param content node of the SystemGuideline matching the clicked node
#' @param nsIndex index of the used namespaces in the current module.
#'
...
#' @title nt.titles
#'
#' @description Apply a precise action on elements from _vec_ named after _vec_'s names
#'
#' @param vec "mother vector" containing elements
#' @param action_target duo of named elements and 'renderUI' action to perform
#'
#' @importFrom shiny tags pre code HTML
nt.titles <- function(vec, action_target) {
  sapply(
    ...
    function(target) {
      ...
      # validity check
      possibleActions <- c(
        "remove", "replace", "moveback",
        "tocode", "use", "addurl"
      )
      if (!any(sapply(possibleActions, grepl, action))) {
        stop(
          "Action not recognized, must be one of those:\n",
...
#' @title commonPath
#'
#' @description Extract the path made common from every leaf path. Used to get a
#'  non-leaf node's path
#'
#' @param li lists in which path is searched
#' @param name currently selected node's name
...
#' @title has_child
#'
#'  @description check if one of the children of the input list is a list
#'
#'  @param li list whose child(ren) are checked
