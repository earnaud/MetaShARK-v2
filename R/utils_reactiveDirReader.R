#' reactiveDirReader
#'
#' Function adapted from `reactivePoll()`. Used to create a reactive data
...
#' @param full_names (logical) a logical value. If TRUE, the directory path is
#' prepended to the file names to give a relative file path. If FALSE, the file
#' names (rather than paths) are returned.
#' @param intervalMillis (integer) Approximate number of milliseconds to wait
...
#' only returns "No files found." through `validate()`.
#'
#' @export
reactiveDirReader <- function(repo, session, pattern, full_names = FALSE,
                              intervalMillis = 1000) {
  files <- reactiveVal(character())

  files_poll <- reactivePoll(
    intervalMillis,
    session,
    function() {
    ...
    function() {
      dir(repo, pattern, full.names = full_names)
    }
  )
